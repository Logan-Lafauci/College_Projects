...
instr 1	;;; basic instrument with portamento
	...
	kline  line  0, p3, 100
	kenv   linen p4, 0.01, p3, 0.5

	asig   oscil 1, icps+kline, ift
...
instr 2	;;; 3-oscillator AS
	...
	kenv1	linen 	iamp, 0.1*idur, idur, 0.5*idur
	kenv2	linen 	iamp, 0.3*idur, idur, 0.3*idur
	kenv3	linen 	iamp, 0.5*idur, idur, 0.1*idur
	
	asig1	oscil 	kenv1, icps*.25, ift
	asig2	oscil 	kenv2, icps*.42, ift
	asig3	oscil 	kenv3, icps*.53, ift
	
	kenv		linen	1, iatt, idur, idecay
	aout		= kenv*(asig1 + asig2 + asig3)/3
	
	outs		aout,aout
	
endin

instr 3	;;; 10-oscillators HW
